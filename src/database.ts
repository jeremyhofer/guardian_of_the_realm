...
const sql = new Database('./data/gotr_bot.sqlite');

const playerTable = sql
  .prepare(
    `
...
if (playerTable['count(*)'] === 0) {
  console.log('creating player_data');
...
const resetWars = (): void => {
  console.log('resetting wars');
  sql
    .prepare(
      `
    DELETE FROM wars;
...
if (pactTable['count(*)'] === 0) {
  console.log('creating pacts');
  // If the table isn't there, create it and setup the database correctly.
  sql
    .prepare(
      `
...
if (siegeTable['count(*)'] === 0) {
  console.log('creating sieges');
  // If the table isn't there, create it and setup the database correctly.
  sql
    .prepare(
      `
...
export const operations = {
  ...
  set_player: sql.prepare(`
    INSERT OR REPLACE INTO player_data (
      user, house, men, ships, money, pray_last_time, arson_last_time, pray_last_time, raid_last_time, smuggle_last_time,
