...
export async function interactionHandler(
  ...
  if (commandName in commandDispatch) {
    ...
    if (
      ...
      !(
        interaction.guild?.members.cache
          ...
          ?.roles.cache.has(assets.developerRole) ?? false
      )
    ) {
    ...
    if (
      'allowed_channels' in commandConfig &&
      !(
        commandConfig.allowed_channels?.includes(
    ...
    if ('cooldown' in commandConfig) {
      ...
      if (cooldownField !== undefined) {
        ...
        const baseReply: string = commandConfig.cooldown?.reply ?? '';
    ...
    if ('cooldown_from_start' in commandConfig) {
      ...
      const cooldownTime: number = commandConfig.cooldown_from_start ?? 0;
    ...
    if (cooldown && cooldownField !== undefined) {
       ...
       * cooldown here
       */
      await Database.playerData.updateCommandLastTime(
        interaction.user.id,
        cooldownField,
        currentTime
      );
